...
struct PostOutType {
    ...
    std::vector<std::vector<bm::DetectBox>> results; // a batch(vector) of images(vector) of det bboxes(DetectBox)
};
...
//    saveImage(cfg.resizedImages[0], "resize.jpg");
    ...
    // saveImage(cfg.preOutImages[0], "preout.jpg");
...
bool yoloV3BoxParse(bm::DetectBox& box, float* data, size_t len, float probThresh, CoordConvertInfo& ci){
...
/*
    allPredictions: 
        std::map<image_id, bboxes>
        all boxes in a dataset
        output
    out:
        singel batch input
*/
bool resultProcess(const PostOutType& out, 
                    std::map<int, std::vector<DetectBox>>& allPredictions){
    if(out.rawIns.empty()) return false;
    auto batch = out.rawIns.size();
    for(auto b=0; b<batch; b++){
        int img_id = 0;
        std::vector<DetectBox> img_boxes; 
        ...
        img_boxes.insert(img_boxes.end(), out.results[b].begin(), out.results[b].end());
        allPredictions[img_id] = img_boxes;
        // std::cout<<"allPredictions.size()======="<<allPredictions.size()<<std::endl;
    }
    return true;
}
int main(int argc, char* argv[]){
    ...
    std::string bmodel = topDir + "models/yolov3/fix8b_b4.bmodel";
    ...
    std::thread resultThread([&runner, &info, &allPredictions, &refFile](){
        PostOutType out;
        std::shared_ptr<ProcessStatus> status;
        bool stopped = false;
        thread_local static std::list<std::vector<bm::DetectBox>> sorted_outputs;
        while(true){
            ...
            if(runner.allStopped()){
                mAP map_obj;
                // allPredictions: std::vector<DetectBox>
                float mAP_val = map_obj.calc(allPredictions, refFile); 
                std::cout << "**************** The mAP is " << std::setprecision(3) << mAP_val << std::endl;
                info.show();
                break;
            }
        }

...
def get_args():
    ...
    defaults = SUPPORTED_PROFILES["resnet50"]
...
class RunnerBase:
    ...
    def process_result(self, qitem, task_id_res, results, valid):
        try:
            if not valid:
                raise RuntimeError("bmservice exception, error {}".format(valid))
    ...
    def run_one_item(self, qitem):
        # run the prediction
        processed_results = []
        task_id = self.service.put(qitem.img)
...
def main():
    ...
    image_format = args.data_format = 'NCHW'
    ...
    print(mlperf_conf, args.model_name, args.scenario)
